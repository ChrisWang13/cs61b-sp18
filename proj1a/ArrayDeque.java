...
public class ArrayDeque<T> {
  ...
  private  void resize(int newSize) {
    ...
    if (front < rear) {
      ...
      for (int new_idx1 = 0; new_idx1 < end1; ++new_idx1) {
        newArray[new_idx1] = items[new_idx1 + diff1];
      }
      // copy [0, rear) to [capacity - front, capacity - front + rear).
      int diff2 = front - capacity;
      int new_start2 = capacity - front;
      int end2 = capacity - front + rear;
      for (int new_idx2 = new_start2; new_idx2 < end2; ++new_idx2) {
        newArray[new_idx2] = items[new_idx2 + diff2];
      }
    }
    assert front != rear;
    items = newArray;
